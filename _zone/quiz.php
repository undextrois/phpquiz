...
$sfqw = $sfq.' WHERE';
	...
	if (isset($HTTP_SESSION_VARS['answered'])) 
	{ 
		$ansd = count($HTTP_SESSION_VARS['answered']); 
	} 
	else 
	{ 
		$ansd = 0; 
...
<?php
	if (isset($HTTP_SESSION_VARS['scoreboard']) && ($HTTP_SESSION_VARS['scoreboard'][$testsub])) 
	{
		...
		{ 
			...
			if ($totsub > 1)
			{ 
				echo '<a href="results.php">Results Sheet</a>'; 
			}
	...
	if (($tot < $limit) && (!isset($HTTP_SESSION_VARS['answered']))) 
	{ 
		echo '<h3">There are only '.$tot.' questions in this category.</h3>'; 
	}
	else if  (($tot < $limit) && ($tot > 0) && ($ansd > 0)) 
	{ 
		echo '<h3">Only '.$tot.' questions remaining in this category.</h3>'; 
	}
	else if (($tot == 0) && ($ansd > 0)) 
	{ 
		echo '<h3>There are no more questions in this category.</h3>'."\n"; 
	}
	if ($tot > 0) 
	{
?>
	<span class="Heading4">Please answer the following 
							<?php $qu = ' question'; 
									if ($tot > 1) 
									{ 
										echo $tot.$qu.'s'; 
									...
									{ 
										echo $qu; 
							}?> and then press the <em>Mark Test</em> button below!</span>
<?php } ?>
	...
	$qi = 0;
		...
		$cor = $qry['corans'];
			if (!ereg('ans', $cor))
			{
				...
				switch ($asl) 
				{
					case 4: $answers = array('1', '2', '3', '4'); 
							break;
					case 5: $answers = array('1', '2', '3', '4', '5'); 
							break;
					case 6: $answers = array('1', '2', '3', '4', '5', '6'); 
							break;
